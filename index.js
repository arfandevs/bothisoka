/**
   ...
   * Contact Me on wa.me/6281410528015
   * Follow https://github.com/arfandevs
*/
    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                // Get Profile Picture Group
                let ppny = await getBuffer(ppuser)
                try {
...
document: { url: 'https://wa.me/6281410528015' },
mimetype: 'video/3gpp',
...
hisoka.sendMessage(anu.id, {
buttons: [{buttonId: `Thx Lui`, buttonText: { displayText: "Bye" }, type: 1 }],
document: { url: 'https://wa.me/6281410528015' },
mimetype: 'video/3gpp',
jpegThumbnail: await hisoka.reSize(ppny, 300, 300),
fileName: `Number ${PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international')} Out!`,
fileLength: 3221000000,
pageCount: 1222222, // berapa halaman?
caption: `-1 Beban`,
footer: `Hisoka - Morou By AhmdLui`,
contextInfo: { mentionedJid: [m.sender],
externalAdReply: {
title: `Leaving To ${metadata.subject}`,
body: ``,
mediaType: 1,
thumbnail: await getBuffer(ppuser),
renderLargerThumbnail: true,
...
mediaUrl: `ppuser`
    ...
    // Setting
        hisoka.parseMention = async (text = '') => {
    ...
    hisoka.getName = (jid, withoutContact  = false) => {
        ...
        else v = id === '081410528015@s.whatsapp.net' ? {
            id,
            name: 'WhatsApp'
 ...
 let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
     ...
     * @returns
     */
         hisoka.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      const buttons = [
      ...
      const buttonMessage = {
        document: { url: "https://wa.me/6281410528015" },
        mimetype: doc,
        fileName: await ucapan(),
        fileLength: 887890909999999,
        pageCount: 1234567890123456789012345,
        caption: content,
        footer: footerText,
        mentions: await hisoka.parseMention(content + footerText),
        ...options,
        buttons: buttons,
        headerType: 1
      }
      hisoka.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, forwardingScore: 99999, isForwarded: true, ...options })
    }
    hisoka.send2ButtonDoc = async (jid, content, footerText, button1, id1, button2, id2, quoted, options) => {
        ...
        hisoka.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentionedJid: hisoka.parseMention(content + footerText), forwardingScore: 99999, isForwarded: true }, ...options, ephemeralExpiration: 86400 } )
    }
    hisoka.sendButtonimgDoc = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        ...
        const buttonMessage = {
            jpegThumbnail: await hisoka.reSize(file, 300, 200),
            document: { url: "https://wa.me/6281410528015"},
            mimetype: doc,
            fileName: await ucapan(),
            fileLength: 887890909999999,
            ...
            caption: contentText,
            footer: footerText,
            mentions: await hisoka.parseMention(contentText + footerText),
            ...options,
            buttons: buttons,
            headerType: 1
        }

        return await hisoka.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentionedJid: hisoka.parseMention(contentText + footerText) }, ...options })
    }
    hisoka.send2ButtonimgDoc = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
    ...
    hisoka.sendButton = async (jid, content, footerText, button1, id1, quoted, options) => {
        const buttons = [
        { buttonId: id1, buttonText: { displayText: button1 }, type: 1 },
        ]
        const buttonMessage = {
        ...
        hisoka.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentionedJid: hisoka.parseMention(content + footerText), forwardingScore: 999999, isForwarded: true }, ...options })
    }
    hisoka.send2Button = async (jid, content, footerText, button1, id1, button2, id2, quoted, options) => {
        const buttons = [
        { buttonId: id1, buttonText: { displayText: button1 }, type: 1 },
        { buttonId: id2, buttonText: { displayText: button2 }, type: 1 }
        ]
    ...
    hisoka.send3Button = async (jid, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) => {
        const buttons = [
        { buttonId: id1, buttonText: { displayText: button1 }, type: 1 },
        { buttonId: id2, buttonText: { displayText: button2 }, type: 1 },
        ...
        hisoka.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentions: hisoka.parseMention(content + footerText), forwardingScore: 99999, isForwarded: true }, ...options })
    }
        hisoka.sendBut = async (jid, content, footerText, button1, id1, quoted, options) => {
        const buttons = [
        { buttonId: id1, buttonText: { displayText: button1 }, type: 1 },
        ]
        const buttonMessage = {
    ...
    hisoka.send2But = async (jid, content, footerText, button1, id1, button2, id2, quoted, options) => {
        const buttons = [
        { buttonId: id1, buttonText: { displayText: button1 }, type: 1 },
        { buttonId: id2, buttonText: { displayText: button2 }, type: 1 }
        ]
     ...
     */
    hisoka.sendButtonLoc = async (jid, buffer, content, footer, button1, row1, quoted, options = {}) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        let resizee = await hisoka.reSize(buffer, 300, 150)
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        catch (e) { if (e.json) throw e.json }
        }
        ...
        let buttonMessage = {
            location: { jpegThumbnail: resizee },
            caption: content,
            footer: footer,
            mentions: await hisoka.parseMention(content + footer),
            ...options,
            buttons: buttons,
            headerType: 6
        }
        return await  hisoka.sendMessage(jid, buttonMessage, {
            quoted,
    ...
    hisoka.send2ButtonLoc = async (jid, buffer, content, footer, button1, row1, button2, row2, quoted, options = {}) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        let resizee = await hisoka.reSize(buffer, 300, 150)
        if (res && res.status !== 200 || file.length <= 65536) {
    ...
    hisoka.send3ButtonLoc = async (jid, buffer, content, footer, button1, row1, button2, row2, button3, row3, quoted, options = {}) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        let resizee = await hisoka.reSize(buffer, 300, 150)
        if (res && res.status !== 200 || file.length <= 65536) {
     ...
     * @param {Object} options
     */
    hisoka.sendButtonImg = async (jid, buffer, contentText, footerText, button1, id1, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
...
hisoka.send2ButtonImg = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
    ...
    hisoka.send3ButtonImg = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, button3, id3, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
     ...
     * @param {Object} options
     */
    hisoka.sendButtonVid = async (jid, buffer, contentText, footerText, button1, id1, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        ...
        const buttonMessage = {
            ...
            caption: contentText,
            footer: footerText,
            mentions: await hisoka.parseMention(contentText),
            ...options,
    ...
    hisoka.send2ButtonVid = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        ...
        let buttons = [
        { buttonId: id1, buttonText: { displayText: button1 }, type: 1 },                                                     { buttonId: id2, buttonText: { displayText: button2 }, type: 1 }
        ]
        const buttonMessage = {
            video: file,
            fileLength: 887890909999999,
    ...
    hisoka.send3ButtonVid = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, button3, id3, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        ...
        const buttonMessage = {
            video: file,
            //fileLength: 887890909999999,                                                                                        caption: contentText,
            footer: footerText,
            mentions: await hisoka.parseMention(contentText + footerText),
            ...options,
            buttons: buttons,                                                                                                     headerType: 4
    ...
    * @param {Object} quoted
    */
    hisoka.send3TemplateButtonImg = async (jid, buffer, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
    ...
    hisoka.sendTemplateButtonDoc = async (jid, buffer, content, footerText, button1, id1, quoted, options) => {                 const buttons = [
            { index: 1, urlButton: { displayText: dtu, url: urlnya } },
    ...
    hisoka.sendTemplateButtonLoc = async (jid, buffer, contentText, footer, buttons1, row1, quoted, options) => {           const type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        catch (e) { if (e.json) throw e.json }                                                                                }
    ...
    }), { userJid: hisoka.user.jid, quoted: quoted, contextInfo: { mentionedJid: hisoka.parseMention(contentText + footer) }, ephemeralExpiration: 86400, ...options });
    ...
    hisoka.sendTemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        ...
        hisoka.sendMessage(jid, key, { ephemeralExpiration: 86400, mentions: hisoka.parseMention(content + footerText), contextInfo: { forwardingScore: 99999, isForwarded: true }, ...options })
    }
    hisoka.send2TemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, btn2, id2, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        ...
        hisoka.sendMessage(jid, key, { quoted, ephemeralExpiration: 86400, contextInfo: { mentions: hisoka.parseMention(content + footerText), forwardingScore: 9999, isForwarded: true } })
    }
    hisoka.send3TemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, btn2, id2, btn3, id3, quoted, options) => {
        let type = await hisoka.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {                                                              try { throw { json: JSON.parse(file.toString()) } }
        catch (e) { if (e.json) throw e.json }
        }
        const key = {
            video: file,
            ...
            { index: 2, callButton: { displayText: dtc,  phoneNumber: phn } },
            { index: 3, quickReplyButton: { displayText: btn1, id: id1 } },
            { index: 4, quickReplyButton: { displayText: btn2, id: id2 } },                                                       { index: 5, quickReplyButton: { displayText: btn3, id: id3 } }
            ]
        }
        hisoka.sendMessage(jid, key, { quoted, ephemeralExpiration: 86400, contextInfo: { forwardingScore: 9999, isForwarded: true, mentions: hisoka.parseMention(content) } })
    }
    ...
    * @returns
    */
    hisoka.sendGroupV4Invite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', options = {}) => {
 ...
 return buffer
  ...
  copy.key.fromMe = sender === hisoka.user.id
    ...
    hisoka.getFile = async (PATH, save) => {
        ...
        filename = path.join(__filename, '../Sampah/' + new Date * 1 + '.' + type.ext)
